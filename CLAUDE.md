...
- All app logic is one self-contained file: `app/src/main/assets/index.html` (vanilla JS, no framework, no build step, no vendored libs).
  - State `S = {v:3, settings:{cur, theme, lastAcc, lastAccIn, lastCheck, remind:{daily,time,dues}, notifAsked}, accounts, cats, txns, loans, assets}`
  ...
  - Other assets: `S.assets = [{id, name, e, value, currency}]` (value only, no entries).
