...
(async () => {
  ...
  await settle();
  const chipBoxes = await page.evaluate(() => [...document.querySelectorAll('.strip .chip')].map(c => ({ text: c.textContent, width: c.getBoundingClientRect().width, scrollWidth: c.scrollWidth })));
  ok(chipBoxes.every(c => c.scrollWidth <= c.width + 1), 'every account chip is wide enough for its own text, no clipping: ' + JSON.stringify(chipBoxes));
  const stripOverflow = await page.evaluate(() => { const s = document.querySelector('.strip'); return { clientWidth: s.clientWidth, scrollWidth: s.scrollWidth }; });
  ok(stripOverflow.scrollWidth > stripOverflow.clientWidth, 'the filter row overflows and scrolls instead of squeezing chips: ' + JSON.stringify(stripOverflow));
  ...
  await page.evaluate(() => { document.querySelector('.strip').scrollLeft = 9999; });
  await settle();
  await page.screenshot({ path: OUT + '/history-chips-scrolled.png' });
  const lastChip = await page.evaluate(() => { const cs = [...document.querySelectorAll('.strip .chip')]; const c = cs[cs.length - 1]; return { text: c.textContent, width: c.getBoundingClientRect().width, scrollWidth: c.scrollWidth }; });
  ok(lastChip.scrollWidth <= lastChip.width + 1, 'scrolled into view, the last chip is unclipped too: ' + JSON.stringify(lastChip));
  ...
  const keyHeight = await page.evaluate(() => parseFloat(getComputedStyle(document.querySelector('.calc button:not(.calcbtn)')).height));
  ok(keyHeight >= 50, 'calculator keys are taller than before (>=50px): ' + keyHeight);
  await act('calc-toggle', 'f-amt'); await settle();
  const openState = await page.evaluate(() => { const inp = document.getElementById('f-amt'); return { active: document.activeElement === inp, readOnly: inp.readOnly, inputmode: inp.getAttribute('inputmode') }; });
  ...
  ok(openState.inputmode === 'none', 'inputmode is "none" while the calculator is open, suppressing the system keyboard');
  await page.screenshot({ path: OUT + '/calc-taller-caret.png' });
  await act('calc-toggle', 'f-amt'); await settle();
  const closedState = await page.evaluate(() => { const inp = document.getElementById('f-amt'); return { readOnly: inp.readOnly, inputmode: inp.getAttribute('inputmode') }; });
  ok(closedState.readOnly === false && closedState.inputmode === 'decimal', 'after closing, the field is editable again with the normal decimal keyboard: ' + JSON.stringify(closedState));
  await act('close');
  ...
  ok(await page.inputValue('#f-amt') === '100', 'the in-progress calculation (50×2) was applied on back, got ' + (await page.inputValue('#f-amt')));
  await act('close');

  ok(errors.length === 0, 'no page errors ' + JSON.stringify(errors));
  ok(!(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth)), 'no horizontal overflow outside the intentionally-scrollable strip');
