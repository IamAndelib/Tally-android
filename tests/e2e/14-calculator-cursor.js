...
(async () => {
  ...
  const type = async keys => { for (const k of keys) await page.click(`.calc [data-act="calc-key"][data-v="${k}"]`); };
  ...
  async function tapAtOffset(id, offset) {
    const rect = await page.evaluate(({ id, offset }) => {
      const mirror = document.getElementById('cm-' + id);
      let acc = 0, node = null, localOffset = 0;
      for (const n of mirror.childNodes) {
        if (n.nodeType === 3) {
          if (offset <= acc + n.textContent.length) { node = n; localOffset = offset - acc; break; }
          acc += n.textContent.length;
        }
      }
      if (!node) { const last = [...mirror.childNodes].reverse().find(n => n.nodeType === 3); node = last || mirror.firstChild; localOffset = node && node.textContent ? node.textContent.length : 0; }
      const range = document.createRange();
      range.setStart(node, localOffset); range.setEnd(node, localOffset);
  ...
  async function clearAll(id) {
    ...
    for (let i = 0; i < 20 && (await page.inputValue('#' + id)) !== ''; i++) await page.click(`.calc [data-act="calc-key"][data-v="⌫"]`);
  }
  ...
  await type(['9']);
  ok(await page.inputValue('#f-amt') === '1209345', 'tapping mid-value then typing 9 inserts there, not at the end: ' + (await page.inputValue('#f-amt')));

  // ---- 2. backspace at a repositioned cursor removes the char just before it, not the last char overall
  await type(['⌫']);
  ok(await page.inputValue('#f-amt') === '120345', 'backspace after the mid-insert removes that inserted digit, restoring 120345: ' + (await page.inputValue('#f-amt')));
  await tapAtOffset('f-amt', 3);
  await type(['⌫']);
  ok(await page.inputValue('#f-amt') === '12345', 'backspace with the cursor mid-string removes the char before it (the "0"), not the last char: ' + (await page.inputValue('#f-amt')));
  ...
  await type(['+']);
  ok(await page.inputValue('#f-amt') === '123+45', 'operator typed mid-string splits the expression there: ' + (await page.inputValue('#f-amt')));
  ...
  await type(['×']);
  ok(await page.inputValue('#f-amt') === '123×45', 'a second operator typed right after the first replaces it, mid-string too: ' + (await page.inputValue('#f-amt')));
  ...
  await type(['.']);
  ok(await page.inputValue('#f-amt') === '1.2+3', 'a second "." inside a segment that already has one is ignored: ' + (await page.inputValue('#f-amt')));
  await tapAtOffset('f-amt', 5); // end, inside "3" — a different number segment, no dot yet
  await type(['.']);
  ok(await page.inputValue('#f-amt') === '1.2+3.', 'a "." in a different, dot-free segment is still allowed: ' + (await page.inputValue('#f-amt')));
  ...
  await type(['8']);
  ok(await page.inputValue('#f-amt') === '778', 'tapping past the end still appends at the end: ' + (await page.inputValue('#f-amt')));

  await page.screenshot({ path: OUT + '/calc-cursor.png' });
  await act('calc-toggle', 'f-amt'); await settle();
  ok(await page.inputValue('#f-amt') === '778', 'closing evaluates the final expression correctly: ' + (await page.inputValue('#f-amt')));
  await act('close');
