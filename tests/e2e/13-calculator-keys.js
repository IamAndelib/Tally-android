...
(async () => {
  ...
  const keyClasses = await page.evaluate(() =>
    Object.fromEntries(
      [...document.querySelectorAll('.calc [data-act="calc-key"]')].map(b => [b.dataset.v, b.className])
    )
  );
  ok(keyClasses["−"] === "op", 'minus is classed "op" like the other operators, got "' + keyClasses["−"] + '"');
  const opColors = await page.evaluate(() =>
    ["+", "×", "÷", "−"].map(
      v =>
        getComputedStyle([...document.querySelectorAll('.calc [data-act="calc-key"]')].find(b => b.dataset.v === v))
          .color
    )
  );
  ok(
    new Set(opColors).size === 1,
    "all four operator keys (+ × ÷ −) render the exact same colour: " + JSON.stringify(opColors)
  );
  ...
  ok(
    delColor !== digitColor,
    "backspace colour differs from the neutral digit keys: del=" + delColor + " digit=" + digitColor
  );
  ok(
    delColor !== opColors[0],
    "backspace colour differs from the operator keys too: " + delColor + " vs " + opColors[0]
  );
  ...
  const midState = await page.evaluate(() => {
    const inp = document.getElementById("f-amt"),
      box = inp.closest(".amtbox"),
      mirror = document.getElementById("cm-f-amt");
    return {
      calcing: box.classList.contains("calcing"),
      mirrorHidden: mirror.hidden,
      mirrorHasBlink: !!mirror.querySelector(".blink"),
  ...
  ok(
    midState.inputColor === "rgba(0, 0, 0, 0)" || midState.inputColor === "transparent",
    "the real input's text is made transparent so only the mirror shows: " + midState.inputColor
  );
  ...
  ok(
    afterState.inputColor !== "rgba(0, 0, 0, 0)" && afterState.inputColor !== "transparent",
    "the real input's text is visible again after closing: " + afterState.inputColor
  );
