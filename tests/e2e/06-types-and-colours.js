...
(async () => {
  ...
  await page.evaluate(
    t =>
      localStorage.setItem(
        ...
        JSON.stringify({
          v: 4,
          settings: { cur: "CAD", lastCheck: t },
          accounts: [
            { id: "k", name: "Bkash", type: "wallet", currency: "CAD", opening: 5000 },
            { id: "w", name: "Wallet", type: "cash", currency: "CAD", opening: 200 },
          ],
          txns: [{ id: "e1", ts: 1, date: t, type: "expense", amount: 12, account: "w", cat: "food", note: "" }],
        })
      ),
    t
  );
  ...
  const pressed = async sel => {
    ...
    const r = await page.evaluate(s => {
      ...
      return {
        ...
        layer: +getComputedStyle(e, "::before").opacity,
        rip: !!e.querySelector(".rip"),
      };
    }, sel);
    await T("touchMove", x, y + 30);
    await T("touchEnd");
    await page.waitForTimeout(80);
    return r;
  };
  for (const sel of [".fab.minus", ".balbar", ".fbtn.loan"]) {
    const r = await pressed(sel);
    ok(
      r.tr === "none" && !r.rip && r.layer > 0.05,
      sel + ": subtle state layer only while pressed " + JSON.stringify(r)
    );
  }
  await tap('[data-act="tab"][data-v="assets"]');
  ok(
    await page.evaluate(() => getComputedStyle(document.getElementById("app")).animationName === "none"),
    "no page animation on tab switch"
  );
  {
    const r = await pressed('[data-src="acc"] .tx');
    ok(r.tr === "none", "row: no scale while pressed");
  }
  ...
  ok(
    !(await page.$("#pd")) && (await page.textContent(".pnav .plabel")).includes("–"),
    "second tap picks the end: " + (await page.textContent(".pnav .plabel"))
  );
  // a real drag still works
  await tap(".pnav .plabel");
  await tap('#pd [data-act="pd-tab"][data-v="range"]');
  {
    const p1 = await box(`#pd [data-act="pd-rday"][data-v="${rdays[0]}"]`),
      p2 = await box(`#pd [data-act="pd-rday"][data-v="${rdays[Math.min(3, rdays.length - 1)]}"]`);
    ...
    ok(
      !(await page.$("#pd")) && (await page.textContent(".pnav .plabel")).includes("–"),
      "dragging across days picks a range"
    );
  }
  ...
  await page.fill("#nt-name", "Crypto");
  await page.screenshot({ path: OUT + "/type-dialog.png" });
  ...
  await page.fill("#f-name", "Binance");
  await page.fill("#f-open", "300");
  ...
  let S = await state();
  const cr = S.types.find(x => x.name === "Crypto"),
    bn = S.accounts.find(x => x.name === "Binance");
  ...
  S = await state();
  ok(
    S.types.some(x => x.id === cr.id) && S.accounts.find(x => x.id === bn.id).type === cr.id,
    "type survives a reload (migrate)"
  );
  ...
  ok(
    (await page.textContent("#snack")).includes("used by 1"),
    "a used type cannot be removed: " + (await page.textContent("#snack"))
  );
  await tap('#sheet [data-act="type-new"]');
  await page.fill("#nt-name", "Temp");
  await tap('[data-act="nt-add"]');
  ...
  S = await state();
  ok(
    !S.types.some(x => x.id === tmp.id) && !(await page.$(`#sheet .chip[data-v="${tmp.id}"]`)) && dialogs === 0,
    "an unused type is removed after confirming (no browser dialog)"
  );
  ...
  await tap('[data-act="hx-ok"]');
  ok(
    (await page.$eval('#sheet .dot[data-v="#1122aa"]', e => e.getAttribute("aria-pressed"))) === "true",
    "custom dot added and selected"
  );
  await page.evaluate(() => document.querySelector("#f-dots").scrollIntoView({ block: "center" }));
  await page.screenshot({ path: OUT + "/dots.png" });
  await tap('#sheet [data-act="cat-save"]');
  S = await state();
  ok(
    S.cats.find(c => c.id === "groceries").c === "#1122aa" && S.settings.customCols[0] === "#1122aa",
    "category saved with the custom colour; remembered"
  );
