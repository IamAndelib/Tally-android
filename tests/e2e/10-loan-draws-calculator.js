...
(async () => {
  ...
  const d = await page.evaluate(() => {
    ...
    return { t: f(0), m2: f(-2), p5: f(5), p10: f(10), p13: f(13) };
  });
  await page.evaluate(
    d =>
  ...
  await act("prev");
  await act("prev");
  await settle(); // Home now on d.m2
  await act("loan-new", "borrow");
  await settle();
  ok(
    (await page.evaluate(() => document.getElementById("f-date").dataset.v)) === d.m2,
    "loan form date follows Home's viewed day, not today"
  );
  await act("close");
  await act("next");
  await act("next");
  await settle(); // back to today

  // ---- 2. suggested people + merge: lend 3000 to Swarna due in 5 days
  await act("loan-new", "lend");
  await settle();
  await page.fill("#f-person", "Swarna");
  await page.fill("#f-amt", "3000");
  await pickDate("f-due", d.p5);
  ...
  await act("loan-new", "lend");
  await settle();
  await page.fill("#f-person", "Sw");
  await settle();
  let chips = await page.evaluate(() =>
    [...document.querySelectorAll('#f-person-sug [data-act="f-person-pick"]')].map(b => ({
      v: b.dataset.v,
      t: b.textContent,
    }))
  );
  ok(
    chips.length === 1 && chips[0].t === "Swarna" && chips[0].v === L.id,
    "suggestion chip is the open LEND to Swarna only, not the borrow " + JSON.stringify(chips)
  );
  const gapBoxes = await page.evaluate(() => ({
    name: document.getElementById("f-person").getBoundingClientRect(),
    sug: document.getElementById("f-person-sug").getBoundingClientRect(),
  }));
  ok(
    gapBoxes.sug.top - gapBoxes.name.bottom >= 8,
    "suggestion chips have a visible gap below the name field: " +
      (gapBoxes.sug.top - gapBoxes.name.bottom).toFixed(1) +
      "px"
  );

  // currency mismatch: switching to the USD account should drop the suggestion
  await sact("f-acc", "usd");
  await settle();
  ok(await page.isHidden("#f-person-sug"), "suggestion hidden once a currency-mismatched account is picked");
  await sact("f-acc", "w");
  await settle(); // back to a BDT account
  ...
  ok(
    draws[0].amount === 1000 && draws[0].due === d.p10 && draws[1].amount === 3000 && draws[1].due === d.p5,
    "each draw kept its own amount and due date"
  );
  ...
  ok(
    (await page.evaluate(() => document.getElementById("f-due2").dataset.v)) === d.p5,
    "due-date card shows the nearer of the two dues (5 days out)"
  );
  ...
  const mores = await page.evaluate(() =>
    [...document.querySelectorAll("#sheet .tx.exp .more")].map(e => e.textContent)
  );
  ok(
    mores.filter(m => /due/.test(m)).length === 2,
    "both expanded draw rows show their own due date: " + JSON.stringify(mores)
  );
  ...
  const d5draw = draws.find(t => t.amount === 3000),
    d10draw = draws.find(t => t.amount === 1000);
  ...
  await act("tab", "liabs");
  await settle();
  await act("loan-open", borrowSwarna.id);
  await settle();
  await page.click("#sheet .tx.exp");
  await settle();
  const soleDrawButtons = await page.evaluate(() => {
    const m = document.querySelector("#sheet .tx.exp .more");
    return {
      edit: !!m.querySelector('[data-act="loandraw-edit"]'),
      del: !!m.querySelector('[data-act="loanpay-del"]'),
    };
  });
  ok(
    soleDrawButtons.edit && !soleDrawButtons.del,
    "sole remaining draw: Edit present, Delete absent " + JSON.stringify(soleDrawButtons)
  );
  await act("close");
  ...
  await settle();
  ok(
    await page.isVisible('[data-act="calc-toggle"][data-v="f-amt"]'),
    "calculator toggle present on the quick-add amount field"
  );
  ...
  ok(
    Math.abs(calcBox.bottom - vh) < 2,
    "calculator docks flush to the bottom of the viewport, like a keyboard: bottom=" +
      calcBox.bottom +
      " viewport=" +
      vh
  );
  ...
  for (const k of ["1", "2", "0", "×", "3", "+", "4", "5"])
    await page.click(`#calc-f-amt [data-act="calc-key"][data-v="${k === "×" ? "×" : k}"]`);
  await act("calc-toggle", "f-amt");
  ok(
    (await page.inputValue("#f-amt")) === "405",
    "calculator computed 120×3+45 = 405, got " + (await page.inputValue("#f-amt"))
  );
  ...
  S = await state();
  ok(
    S.txns.some(t => t.type === "expense" && t.amount === 405),
    "expense of 405 (from the calculator) saved"
  );
  ...
  await act("calc-toggle", "f-amt");
  ok(
    (await page.inputValue("#f-amt")) === "5÷0",
    "divide-by-zero: field left unchanged (no crash, no silent NaN): " + (await page.inputValue("#f-amt"))
  );
  ...
  ok(
    !(await page.$('[data-act="calc-toggle"][data-v="f-toamt"]')),
    "no calculator on transfer's received amount (scope-trimmed)"
  );
  ...
  ok(
    Math.abs(calcBox2.bottom - vh2) < 2,
    "calculator on a long sheet still docks flush to the bottom: bottom=" + calcBox2.bottom + " viewport=" + vh2
  );
  const amtBox2 = await page.evaluate(() => document.getElementById("f-amt").getBoundingClientRect());
  ok(
    amtBox2.top >= 0 && amtBox2.bottom <= calcBox2.top + 1,
    "the payment field, deep in the sheet, still scrolls into view above the calculator"
  );
  await act("calc-toggle", "f-amt");
  await settle();
  await act("close");
  ...
  await settle();
  ok(
    await page.isVisible('[data-act="calc-toggle"][data-v="f-actual"]'),
    'calculator on the "Doesn\'t match?" actual-balance field'
  );
  await act("close");
