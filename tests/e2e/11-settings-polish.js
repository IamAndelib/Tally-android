...
(async () => {
  ...
  await page.evaluate(() =>
    localStorage.setItem(
      "tally:v1",
      JSON.stringify({ v: 6, settings: { cur: "BDT" }, accounts: [], txns: [], loans: [], assets: [] })
    )
  );
  await page.reload();
  await settle();
  ...
  ok(
    about && /Version 1\.0\.42/.test(about.text),
    "About shows the stubbed version: " + JSON.stringify(about && about.text)
  );
  ok(
    about &&
      about.hrefs.includes("https://github.com/IamAndelib") &&
      about.hrefs.includes("https://github.com/IamAndelib/Tally-android"),
    "About links to the GitHub profile and repo: " + JSON.stringify(about && about.hrefs)
  );
  ...
  await settle();
  await page.evaluate(() => {
    delete window.Android.getVersion;
  });
  await act("go", "settings");
  await settle();
  ...
  ok(
    about2 && !/undefined/i.test(about2) && !/Version/.test(about2),
    'About degrades quietly with no version when the bridge lacks getVersion: "' + about2 + '"'
  );
  ...
  ok(
    iconBoxes.appbar && Math.abs(iconBoxes.appbar.width - iconBoxes.appbar.height) < 0.5,
    "appbar icon button is square (circle, not oval): " + JSON.stringify(iconBoxes.appbar)
  );
  ...
  await page.reload();
  await settle();
  const pnavBox = await page.evaluate(() => {
    const el = document.querySelector(".pnav .icon");
    const r = el.getBoundingClientRect();
    return { width: r.width, height: r.height };
  });
  ok(
    Math.abs(pnavBox.width - pnavBox.height) < 0.5,
    "pnav (period nav) icon button is square (circle, not oval): " + JSON.stringify(pnavBox)
  );
  ...
  ok(
    Math.abs(align.dayRight - align.amtRight) < 1.5,
    "History: day-header total lines up with the row amounts below it: " + JSON.stringify(align)
  );
